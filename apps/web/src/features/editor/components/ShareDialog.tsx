...
export function ShareDialog({ open, onClose, noteId, noteTitle, workspaceId }: ShareDialogProps) {
  ...
  const [shares, setShares] = useState<NoteShareDto[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadShares() {
      ...
      setError(null);
      try {
        const data = await sharingApi.listShares(accessToken ?? '', workspaceId, noteId);
  ...
  }, [open, accessToken, workspaceId, noteId]);
  ...
  }, [accessToken, email, permission, workspaceId, noteId]);
  ...
  const handleCreateLink = useCallback(
    ...
    [accessToken, workspaceId, noteId],
  );
