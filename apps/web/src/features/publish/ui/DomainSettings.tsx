...
import { useDomainStore, type DomainVerificationStatus } from '../model/domain-store';
...
const domainKeys = {
  all: ['domain'] as const,
  config: (workspaceId: string) => [...domainKeys.all, 'config', workspaceId] as const,
};
...
export function DomainSettings({ workspaceId, publicSlug }: DomainSettingsProps) {
  ...
  // ── Load domain config via TanStack Query (replaces useEffect + manual fetch) ──

  const { isLoading } = useQuery({
    queryKey: domainKeys.config(workspaceId),
    queryFn: async () => {
