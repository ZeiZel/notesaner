...
import { useReaderCommentsStore } from '../model/reader-comments-store';
import type { PublicReaderComment } from '../model/reader-comments-store';
...
export function ReaderComments({ publicSlug, notePath }: ReaderCommentsProps) {
  ...
  // Effect: reset and fetch initial comments when note changes.
  // TODO: migrate to TanStack Query (useInfiniteQuery) for comment pagination.
  // Kept as useEffect because the store holds shared pagination state.
  useEffect(() => {
    reset();
    void loadComments(1);
  }, [publicSlug, notePath]);
