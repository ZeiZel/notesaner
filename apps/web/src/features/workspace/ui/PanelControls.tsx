...
export function PanelControls({ paneId, className }: PanelControlsProps) {
  const panelState = usePanelControls((s) => s.panelStates[paneId] ?? 'normal');
  const maximize = usePanelControls((s) => s.maximize);
  const minimize = usePanelControls((s) => s.minimize);
  const restore = usePanelControls((s) => s.restore);
