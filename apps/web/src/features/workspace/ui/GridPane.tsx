...
import { usePanelControls, type PanelState } from '../model/PanelControlsContext';
...
export function GridPane({ pane, children }: GridPaneProps) {
  ...
  const panelState = usePanelControls((s) => (s.panelStates[pane.id] ?? 'normal') as PanelState);
  const isPaneVisible = usePanelControls((s) => s.isPaneVisible);
