// NOTE: Mixed store — business data (syncStatus, collaborators, pluginStatusItems)
// + editor-derived display state (wordCount, characterCount, cursorPosition).
// Not persisted (ephemeral). Zustand is kept because:
//   - Sync status and collaborator data are business state fed by the sync engine.
//   - Plugin status items are registered by external plugins via the SDK.
//   - Multiple unrelated data sources write to this store concurrently.
/**
 * status-bar-store.ts
 *
